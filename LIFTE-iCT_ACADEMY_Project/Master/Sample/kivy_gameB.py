...
class MatchGameApp(MDApp):
    ...
    def show_start_dialog(self):
        ...
        dialog = MDDialog(
            ...
            buttons = [
                MDFlatButton(
                    text = "スタート！",
                    on_release = lambda x:[dialog.dismiss(), self.setup_game_board()] # ダイアログを閉じてゲーム開始
                )
            ]
        )
        dialog.open
    ...
    def setup_game_board(self):
        ...
        game_grid = self.root.ids.game_grid
        # 前のカードがあったら消す
        ...
        # カードを生成してグリッドに追加
        for icon_name in game_icons:
            # GameCardをインスタンス化して変数cardに代入する
            card = GameCard(icon_name = icon_name)
            game_grid.add_widget(card)

        self.root.ids.status_label.text = 'あなたの番です。カードをめくってください。'
    ...
    def on_card_click(self, card):
        # プレイヤーのターンじゃない、カードがめくられている、すでに揃っている
        # この条件のいずれかが満たされている場合はなにもしない
    ...
    def check_match(self):
        # 2枚のカードの絵柄が同じかチェックする。
        if self.first_card.card_icon_name == self.second_card.card_icon_name:
    ...
    def cpu_turn(self):
        ...
        self.can_click = False
        # メッセージを表示する
        self.root.ids.status_label.text = 'CPUがカードをめくっています...'
        # めくることができる裏向きのカードを探す
        available_cards = [
        ...
        cpu_choices = random.sample(available_cards, 2)
        # 1枚目のカードをめくる
        cpu_first_card = cpu_choices[0]
        # 2枚目のカードをめくる 
        cpu_second_card = cpu_choices[1]

        # 1枚目のカードをインスタンス（self）に格納する
        cpu_first_card.flip()
    ...
    def cpu_flip_second(self, card):
        # 2枚目のカードをインスタンス（self）に格納する
        card.flip()
        self.second_card = card
        # カードの判定を行うメッセージを表示
        self.root.ids.status_label.text = 'CPUが2枚目のカードをめくりました。'
        # 0.8秒後にカードを比較する
        Clock.schedule_once(self.check_match, 0.8)
