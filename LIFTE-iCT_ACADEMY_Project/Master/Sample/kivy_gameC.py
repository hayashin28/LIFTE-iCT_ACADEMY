...
class MatchGameApp(MDApp):
    ...
    # ２番目にめくったカード
    second_card = ObjectProperty(None)
    # クリックできるかどうか
    can_click = BooleanProperty(True)
    # 揃ったペアの数
    matched_pairs = 0
    # 全ペアの数
    total_pairs = 0
    # どちらのターンか ’player’ or 'cpu'
    current_turn = StringProperty('player')
    ...
    def chow_win_dialog(self, winner):
        # ゲームクリアのダイアログ
        if winner == 'player':
