...
class GameCard(MDCard):
    ...
    def unflip(self):
        ...
        if not self.is_matched:
            ...
            # 裏返しの色
            self.md_bg_color = self.themecls.secondary_light
...
class MatchGameApp(MDApp):
    ...
    def show_start_dialog(self):
        ...
        dialog = MDDialog(
            ...
            buttons = [
                MDFlatButton(
                    text = 'スタート！',
                    on_release = lambda x:[dialog.dismiss(), self.setup_game_bord()] # ダイアログを閉じてゲーム開始！
                )
            ]
        )
        
    def setup_game_bord(self):    
        # ゲームで使うアイコンリスト
        icons = ['flower', 'cat', 'dog', 'fich', 'star', 'heart', 'apple', 'car']
        # 各アイコンを２つずつ用意してペアにする
        game_icons = icons * 2 # 16枚のカードにする
        # カードの並びをシャッフルする
        ...
        self.root.ids.status_label.text = 'あなたのターン！'        
    ...
    def on_card_click(self, card):
        # プレイヤーのターンでない、カードがめくられている、または既に揃っている
        # いずれかの条件を満たしていたら何もしない
        ...
        if self.first_card is None:
            ...
            # カードの判定を行うメッセージを表示
            self.root.status_label.text = '同じ絵柄かな？'
            # 0.8秒後にカードを比較する
    ...
    def cpu_turn(self):
        ...
        if len(available_cards) < 2:
            # ゲーム終了間際などで、カードが足りない場合は何もしないでターン終了
            self.can_click = True
            return
    ...
    def show_win_dialog(self, winner):
        ...
        dialog = MDDialog(
            ...
            button = [
                MDFlatButton(
                    text = 'もう一度',
                    on_release = lambda x: [dialog.dismiss(), self.setup_game_bord]
                )
            ]
        )           
